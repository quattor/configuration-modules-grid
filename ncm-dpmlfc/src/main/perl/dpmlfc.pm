...
my $dm_install_location = "/system/lcg/config/LCG_LOCATION";
my $dm_bin_default = "/opt/lcg/bin";
...
my %xroot_config_rules = (
        "DPM_HOST" => "host:dpm;$line_format_envvar",
        "DPNS_HOST" => "host:dpns;$line_format_envvar",
        "DPM_XROOTD_NOGSI" => "xroot->gsiEnabled:xroot",
        "XRDCONFIG", => "xroot->config:xroot",
       );
...
##########################################################################
  ...
  if ( $config->elementExists($dm_install_location) ) {
    $dm_bin_dir = $config->getElement($dm_install_location)->getValue().'/bin';
  } else {
    $dm_bin_dir = $dm_bin_default;
  }
