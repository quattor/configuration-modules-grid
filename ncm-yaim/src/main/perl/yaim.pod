...
An example configuration template C<< yaim_example.pan >> is shipped with
this component.
...
=item * C<< /software/components/yaim/configure >> : boolean

If defined and set to true, the YAIM configuration phase is
triggered. If not, the component will only print what it would do.

=item * C<< /software/components/yaim/force >> : boolean
...
=item * C<< /software/components/yaim/nodetype >> : list of strings
...
parsing the information in the files in directory C<< node-info.d >>. These files are
...
=item * C<< /software/components/yaim/conf/<X> >> : string
...
configuration file under C<< /etc/lcg-quattor-site-info.def >>:

  CE_HOST="myce01.mydomain.org"


You can provide a script in C<< /usr/libexec/ >> called C<< create-YAIM-users_conf >>,
which is executed if it exists with the full name of the C<< users.conf >>
file as an argument. Here you can add your own script to create the
C<< users.conf >> file. The same is true for the C<< groups.conf >>
file, it is called C<< create-YAIM-groups_conf >>.

If no C<< users.conf >> resp. C<< groups.conf >> exists, this YAIM component
will copy over the example ones, otherwise a configuration of gLite 3.0 will fail.

=item * C<< /software/components/yaim/vo/<list>/services/<X> >>

(Note: for backwards compatibility, VO info may be found also under
       C<< /system/vo >> if C<< /software/components/yaim/vo >> is not defined)
...
value C<< USE_VO_D >> is defined and true, the configuration per VO is written
to a file C<< vo.d/<VO_NAME> >> under the directory containing the SITE_INFO file.

Example (assuming that C<< /software/components/yaim/vo/0 >> is ATLAS):

  "/software/components/yaim/vo/0/services/SW_DIR"="/pool/atlas";

This will generate in the yaim config file C<< /etc/lcg-quattor-site-info.def >> :

  VO_ATLAS_SW_DIR="/pool/atlas"

=item * C<< /software/components/yaim/conf/FTA/<X> >>

Sets a specific FTA specific configuration setting <X>. Because the number
of possible keys is almost unlimited Any entry for <X> will be taken.
...
=item * C<< /software/components/yaim/conf/FTS/<X> >>
...
=item * C<< /software/components/yaim/conf/CE/<X> >>
...
=item * C<< /software/components/yaim/conf/CE/closeSE/<SE>/<X> >>
...
Assuming that C<< /software/components/yaim/conf/CLASSIC_HOST >> is
"se.mydomain.org" this settings results in:
...
=item * C<< /software/components/yaim/SCAS/<X> >>
...
=item * C<< /software/components/yaim/GLEXEC/<X> >>
...
=item * C<< /software/components/yaim/vo >>

If this (ordered) list exists, only these VOs are taken into account,
in the given order. This was implemented as required to set up the LCF
services with this YAIM component. If this branch of the schema does not
...
It is possible to add arbitrary information to the YAIM input file by putting
it into the global schema under:
...
To avoid putting security relevant information, like database passwords into CDB,
this component is now adding the file C<< /etc/yaim.secretpasswords >> to
the end of the C<< info.def >> file. You can use other methods to put this
file (in mode 600) onto the node. The file name is configurable with the value

  "/software/components/yaim/SECRET_PASSWORDS" = "/etc/.myinvisiblesecret";

=head2 further configurations

The C<< info.def >> file itself is configurable:

  "/software/components/yaim/SITE_INFO_DEF_FILE" = "/etc/glite-quattor-site-info.def";

And it defaults to C<< /etc/lcg-quattor-site-info.def >>.

The path to YAIM is configurable, which allows to use this component
for LCG as well as for gLite:
...
This allows to use wrappers for Yaim. By default, C<< YAIM_SCRIPT >> is undefined
and the script to be executed is C<< <YAIM_HOME>/bin/yaim >>.
...
C<< /software/components/yaim/* >>.

In previous ncm-yaim versions, C<< /system/vo >> was used for VO
information. This has been moved now to C<< /software/components/yaim/vo >>
...
Savannah under L<https://savannah.cern.ch/bugs/?group=lcgoperation>,
category 'yaim').

The Yaim functions used for any particular node type are defined in
C<< <YAIM_HOME>/node-info.d/* >>. These files are provided as part of
the gLite release and cannot be edited by ncm-yaim in order to
...
the generated configuration file is renamed with suffix C<< .failed >>.
