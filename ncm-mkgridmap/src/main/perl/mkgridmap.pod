...
This list specifies the VO to process. If not present or undefined, defaults to all VOs defined in the configuration (/system/vo).
...
A local grid-mapfile to be imported in the generated grid-mapfile. By default 
this is <edgcfg.location>/etc/grid-mapfile-local
...
user_<local>. This will add mappings to the grid-mapfile-local defined
above.
