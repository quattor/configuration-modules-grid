...
=item C</software/components/globuscfg/sysconfigUpdate> : boolean

If false, C</etc/sysconfig/globus> is not updated. Must be set to
false if you want to manage /etc/sysconfig/globus with another component
(e.g. ncm-sysconfig).

D : true

=item C</software/components/globuscfg/GLOBUS_LOCATION>

The root of the Globus software tree. By default this is C</opt/globus>.

=item C</software/components/globuscfg/GLOBUS_CONFIG>

The full path to the Globus configuration file. Defaults to
C</etc/globus.conf>.

=item C</software/components/globuscfg/globus_flavor_name>

The globus "flavor" to use. There is no default. A typical value is
gcc32dbg.

=item C</software/components/globuscfg/services>

The list of Globus services to restart after a configuration.

=item C</software/components/globuscfg/x509_user_cert>

=item C</software/components/globuscfg/x509_user_key>
...
=item C</software/components/globuscfg/gridmap>
...
=item C</software/components/globuscfg/GLOBUS_TCP_PORT_RANGE>
...
=item C</software/components/globuscfg/GLOBUS_UDP_PORT_RANGE>

The port range that the gatekeeper and gridftp will use to open
up listening services for inbound udp connections. eg 50000,52000

=item C</software/components/globuscfg/gridmapdir>

The path to the grid-mapfile. This has no default as there are some
machines which do not require the security infrastructure to be
installed.

=item C</software/components/globuscfg/mds/globus_flavor_name>

The globus "flavor" to use for MDS will use global value unless specified.

=item C</software/components/globuscfg/mds/user>

The user with which to run the MDS daemons (default: root).

=item C</software/components/globuscfg/mds/x509_user_cert>

=item C</software/components/globuscfg/mds/x509_user_key>

The path to the certificate and key to use for MDS. Host credentials
will be used if not specified.

=item C</software/components/globuscfg/mds/gris/suffix>

The suffix to use for the GRISes. Defaults to C<""Mds-Vo-name=local,o=grid"">.
Double quotes MUST appear as part of the value.

=item C</software/components/globuscfg/mds/gris/provider>
...
=item C</software/components/globuscfg/mds/gris/registration>

The I<list> of GIISes to which the GRISes should register, so it
has entries labelled with numbers. These entries are have
further entries like I<regname>, I<reghost>, I<regport>, I<regperiod>,
and I<ttl>. These are optional. Default values of these parameters
can be changed this using them. The I<regperiod> and
I<ttl> should be specified as a pair with I<ttl> at least twice that
of I<regperiod>. The name of the GIIS defaults
entry I<recordname>, which is obligatory for every element.

=item C</software/components/globuscfg/mds/giis/allowedregs>
...
=item C</software/components/globuscfg/mds/giis/allowedregs/_number_/allowreg>

The list of allowed host:port pairs which may register to this
giis. This is a sub-parameter of the giis.

=item C</software/components/globuscfg/mds/giis/registration>
...
=item C</software/components/globuscfg/gridftp/globus_flavour_name>

The globus "flavor" to use for GridFTP will use global value unless
specified.

=item C</software/components/globuscfg/gridftp/X509_USER_CERT>

=item C</software/components/globuscfg/gridftp/X509_USER_KEY>

The path to the certificate and key to use for GridFTP. Host credentials
will be used if not specified.

=item C</software/components/globuscfg/gridftp/ftpd>

The full path to the GridFTP daemon. Normally this is not specified
as the default is usually correct.

=item C</software/components/globuscfg/gridftp/port>

The port number to use for the GridFTP daemon. The default is 2811.

=item C</software/components/globuscfg/gridftp/umask>

The umask to use for the GridFTP daemon. The default is 002.

=item C</software/components/globuscfg/gridftp/log>

The full path to the log file for the GridFTP daemon. This defaults
to the area C</var/log>.

=item C</software/components/globuscfg/gridftp/user>

The user with which to run the GridFTP daemon. Will default to root.

=item C</software/components/globuscfg/gridftp/options>

This will override all options for the GridFTP daemon. Use only if
you really know what you are doing.

=item C</software/components/globuscfg/gatekeeper/globus_gatekeeper>

The executable name for the gatekeeper.

=item C</software/components/globuscfg/gatekeeper/extra_options>

Additional options to pass to the gatekeeper.

=item C</software/components/globuscfg/gatekeeper/globus_flavor_name>

The globus "flavor" to use for the gatekeeper will use global value
unless specified.

=item C</software/components/globuscfg/gatekeeper/user>

The user name to use to run the gatekeeper.

=item C</software/components/globuscfg/gatekeeper/port>

The port to use for the gatekeeper. (This defaults to 2119 if not
specified.)

=item C</software/components/globuscfg/gatekeeper/logfile>

The location of the log file for the daemon. (Default depends on
whether user is specified.)

=item C</software/components/globuscfg/gatekeeper/jobmanagers>
...
=item C</software/components/globuscfg/gatekeeper/jobmanagers/_entryNo_/recordname>

Obligatory parameter, identifier string for a certain job manager.

=item C</software/components/globuscfg/gatekeeper/jobmanagers/_entryNo_/type>

Mandatory option giving the type of LRMS. E.g. pbs, lsf, etc.

=item C</software/components/globuscfg/gatekeeper/jobmanagers/_enrtyNo_/job_manager>

Name of job manager executable.

=item C</software/components/globuscfg/gatekeeper/jobmanagers/_entryNo_/job_manager_path>

Path to the job manager executable. Only needs to be specified if it
is in a non-standard location.

=item C</software/components/globuscfg/gatekeeper/jobmanagers/_entryNo_/extra_config>
