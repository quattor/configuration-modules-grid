...
=head2 debugLevel : string (one of ERROR, WARNING, INFO, DEBUG, 0 and 1)

Verbosity level. Starting with BDII v5, this is one of the string above. For backward compability
with previous versions, 0 and 1 are also accepted but must be passed as string. Be sure to use
a value appropriate to BDII version used. 
...
=head2 schemas : list of strings (required)
...
Default:  /opt/openldap/libexec/slapd

=head2 slapadd : string

The location of the slapadd executable. Deprecated in BDII v5 and later.

Default:  /opt/openldap/libexec/slapadd
